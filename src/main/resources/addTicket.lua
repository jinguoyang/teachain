...
if point and point ~= nil then
	...
	while (userPoint - usedPoint >= 0.00000001) do
		...
		if timeStamp - startTime >= 0 then
			price = 1 + (sumTicket-sumTicket%1) * 0.00001
		end
	...
	local sstime = timeStamp
	if startTime > timeStamp then
	    sstime = startTime
	end
	if timeCtrls[1] == '1' then
		redis.call('HSET', 'globalDeal', 'endTime', tostring(sstime + countDownMax))
	elseif timeCtrls[1] == '2' then
		local tmpTime = endTime + (countDownIncr * buyTicketNum) - sstime
		if tmpTime > countDownMax then
			tmpTime = countDownMax
		end
		redis.call('HSET', 'globalDeal', 'endTime', tostring(sstime + tmpTime))
	end
